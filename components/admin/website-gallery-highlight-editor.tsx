...
import { saveWebsiteGalleryHighlightCrop } from "@/app/admin/website/actions";
...
/**
 * Admin crop editor for the public Website Gallery Highlight.
 *
 * The editor reuses the exact public formula: the ORIGINAL image is rendered
 * cover-fit inside a 16/9 frame and the stored normalized { x, y, zoom } crop is
 * replayed with a CSS transform - nothing is ever written to the original file.
 * Dragging pans the visible region, wheel/buttons zoom, and pan is clamped so
 * the cover-fit image never reveals an empty edge.
 */
export function WebsiteGalleryHighlightEditor({
  photoId,
  url,
  crop,
  onClose,
  ...
  return (
    <dialog className="admin-dialog crop-editor" onCancel={onClose} ref={dialog}>
      <form action={saveWebsiteGalleryHighlightCrop}>
        <h2>Crop highlight</h2>
        <p>Pan and zoom to frame the selected region. The original image is never modified.</p>
        ...
        <input name="id" type="hidden" value={photoId} />
