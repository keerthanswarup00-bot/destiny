...
-- 3) Official client selections are keyed by profile_id.
...
drop policy if exists "admin manages client selections" on public.client_selection_photos;
...
alter table public.selections drop constraint if exists selections_profile_gallery_photo_unique;
alter table public.selections add constraint selections_profile_gallery_photo_unique
  unique (gallery_id, photo_id, profile_id);

-- 5) Submitted selections associate the client profile.
...
alter table public.selection_submissions drop constraint if exists selection_submissions_profile_gallery_unique;
alter table public.selection_submissions add constraint selection_submissions_profile_gallery_unique
  unique (gallery_id, profile_id);