...
export default async function WatermarkSettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return (
    <section className="admin-content">
      ...
      {error ? <p className="error-note">That image could not be saved — use PNG, SVG, or JPEG up to 5MB.</p> : null}
      ...
      <section className="admin-panel" style={{ marginTop: 24 }}>
        ...
        <p className="placeholder">The watermark is burned into the thumbnail, preview, and download copies generated for every Client Gallery upload. It never touches your private original, which stays available to you alone. Changing or removing the watermark only affects future uploads — existing photos are left exactly as they are.</p>
      </section>
    </section>
  );
}